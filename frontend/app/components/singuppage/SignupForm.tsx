...
export default function SignupForm () {
    ...
    const [confrimPassHidden, setConfrimPassHidden] = useState<boolean>(true);
    ...
    return (
        ...
        {error && 
            ...
            absolute top-[75%] left-[50%] -translate-[50%] border
        ...
        {success && 
            ...
            className="
            flex justify-center items-center gap-2 w-[400px] z-50
            absolute top-[75%] left-[50%] -translate-[50%] bg-white border border-black rounded-2xl p-2 text-black"
            initial={{opacity: 0}} animate={{opacity: 80}} transition={{duration: 0.5}}
            >
                <FaCheckCircle/>
                <p>Sing up successful!</p>
                
            </Motion>
        }
        
        <form className="flex flex-col" onSubmit={handleSubmit(onSubmit)}>
            <input className="border-b border-b-gray-400 p-[10px_0px] outline-none in-dark:border-b-cyan-200" placeholder="Username" type="text" {...register("username", {
            ...
            <div className="h-[20px] text-red-500 flex items-center in-dark:text-red-300">
                {errors.username && <p className="text-[0.8em]">*{errors.username.message}*</p>}
            </div>
            ...
            <div className="relative w-full">
                <input className="w-full border-b border-b-gray-400 p-[10px_0px] outline-none in-dark:border-b-cyan-200" placeholder="Confirm password" 
                type={confrimPassHidden ? "password" : "text"} {...register("confirmPassword", {
                    required: "Confirm password",
                    validate: value => value == getValues("password") || "Password doesn't match"
                })}/>

                <button onClick={() => setConfrimPassHidden(prev => !prev)} type="button" 
                className="absolute right-0 top-[50%] -translate-[50%] text-gray-500">
                    {confrimPassHidden ? <FaEyeSlash/> : <FaEye/>}
                </button>
            </div>
            <div className="h-[20px] text-red-500 flex items-center in-dark:text-red-300">
                {errors.confirmPassword && <p className="text-[0.8em]">*{errors.confirmPassword.message}*</p>}
            </div>
